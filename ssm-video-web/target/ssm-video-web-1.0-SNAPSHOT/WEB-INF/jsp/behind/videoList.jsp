    ...
    <div class="row">
        ...
        <div class="col-md-6">
            ...
            <form id="queryForm"  class="navbar-form navbar-right" action="${pageContext.request.contextPath}/video/list" method="post">
                <input type="text" name="queryTitle" class="form-control" placeholder="标题" value="${queryVo.queryTitle}">
                <div class="btn-group">
                    <button type="button" id="speakerName"
                            class="btn btn-primary dropdown-toggle" data-toggle="dropdown"
                            aria-haspopup="true" aria-expanded="false">
                        <c:forEach items="${speakerList}" var="speaker">
                            <c:if test="${speaker.id == queryVo.querySpeakerId}">
                                ${speaker.speakerName}
                            </c:if>
                        </c:forEach>
                        <c:if test="${empty queryVo.querySpeakerId}">
                            --请选择老师--
                        </c:if>
                    ...
                    <input type="hidden" name="querySpeakerId" id="speakerId" value="${queryVo.querySpeakerId}"/>
                </div>
                <div class="btn-group">
                    <button type="button" id="courseName"
                        ...
                        <c:forEach items="${courseList}" var="course">
                            <c:if test="${course.id == queryVo.queryCourseId}">
                                ${course.courseTitle}
                            </c:if>
                        </c:forEach>
                        <c:if test="${empty queryVo.queryCourseId}">
                            --请选择课程--
                        </c:if>
                        <span class="caret"></span>
                    </button>
                    ...
                    <input type="hidden" name="queryCourseId" id="courseId" value="${queryVo.queryCourseId}"/>
                </div>
...
<div class="container" style="margin-top: 20px;">
    ...
    <form id="form2" action="${pageContext.request.contextPath}/video/delBatchVideos" method="post">
        <table class="table table-bordered table-hover"
            ...
            <tbody>
            <c:forEach items="${video}" var="video" varStatus="status">
                <tr>
                    ...
                    <td>
                        <c:forEach items="${speakerList}" var="speaker">
                            <c:if test="${speaker.id==video.speakerId}">
            ...
            <tr>
                ...
                <td colspan="1" align="center" ><a href="javascript:void(0)" onclick="queryVideo(1)">首页</a></td>
                <td colspan="1" align="center" ><a href="javascript:void(0)" onclick="queryVideo(${page.pageNum-1})">上一页</a></td>
                <td colspan="1" align="center" ><a href="javascript:void(0)" onclick="queryVideo(${page.pageNum+1})">下一页</a></td>
                <td colspan="1" align="center" ><a href="javascript:void(0)" onclick="queryVideo(${page.pages})">尾页</a></td>
                <td colspan="1" align="center" ><a >第${page.pageNum}页</a></td>
                <td colspan="1" align="center" ><a >共${page.pages}页</a></td>
                <td colspan="1" align="center" ><a >共${size}条</a></td>
            </tr>
            </tbody>
        </table>
    </form>
